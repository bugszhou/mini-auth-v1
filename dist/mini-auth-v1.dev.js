(function (global, factory) {
  ...
  */
  ...
  var auth=null;function creatMiniAuth(){var _ref=0<arguments.length&&void 0!==arguments[0]?arguments[0]:{env:"weapp"},appid=_ref.appid,env=_ref.env,url=_ref.url,appKey=_ref.appKey,appSecret=_ref.appSecret;return auth?auth:(auth=miniAuth.create({appid:appid,env:env,tokenReqConfig:{url:url,method:"POST"}}),auth.use("token",function(ctx,next){var jsCode=ctx.tokenReqData.jsCode;jsCode&&(ctx.tokenReqData={js_code:jsCode,sign:defaultSign({js_code:jsCode,app_key:appKey},[appSecret]),app_key:appKey}),next();}),auth.use("afterToken",function(ctx,next){var _ctx$tokenResData$dat=ctx.tokenResData.data,_ctx$tokenResData$dat2=_ctx$tokenResData$dat.data,data=void 0===_ctx$tokenResData$dat2?{}:_ctx$tokenResData$dat2,retcode=_ctx$tokenResData$dat.retcode,msg=_ctx$tokenResData$dat.msg;return 200===retcode?(ctx.tokenResData=data,next()):next({retcode:retcode,msg:msg,data:data})}),auth)}function getToken(){var opts=0<arguments.length&&arguments[0]!==void 0?arguments[0]:{};return new promise$1(function(resolve,reject){function selfGetToken(retry){auth.getToken(opts).then(function(res){resolve(res);})["catch"](function(err){5001===err.errCode||5002===err.errCode?retry>=3?reject(err):selfGetToken(retry+1):reject(err);});}selfGetToken(0);})}
